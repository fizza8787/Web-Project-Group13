    ...
    <div style={{
      ...
      <div style={{ position: 'fixed', inset: 0, overflow: 'hidden', zIndex: 0 }}>
        {[...Array(6)].map((_, i) => (
          <div key={i} style={{
            ...
            width: `${[300,200,400,150,250,350][i]}px`,
            height: `${[300,200,400,150,250,350][i]}px`,
            top: `${[10,60,30,70,20,50][i]}%`,
            left: `${[10,70,50,20,80,40][i]}%`,
            transform: 'translate(-50%, -50%)',
            animation: `float ${[8,6,10,7,9,5][i]}s ease-in-out infinite alternate`,
          }} />
        ))}
      </div>

      <style>{`
        @keyframes float { from { transform: translate(-50%, -50%) scale(1); } to { transform: translate(-50%, -50%) scale(1.1); } }
        ...
        .btn-primary { width: 100%; padding: 14px; background: linear-gradient(135deg, #6366f1, #8b5cf6); border: none; border-radius: 12px; color: white; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.3s; letter-spacing: 0.5px; }
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <h1 style={{ color: 'white', fontSize: '26px', fontWeight: 700, margin: 0 }}>FreelanceHub</h1>
          <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '14px', marginTop: '6px' }}>Client Portal — Sign in to continue</p>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '24px', color: 'rgba(255,255,255,0.4)', fontSize: '14px' }}>
          New here?{' '}
          <Link to="/register" style={{ color: '#818cf8', textDecoration: 'none', fontWeight: 600 }}>Create an account</Link>
        </p>
      </div>
    </div>
  );
